...
namespace SpreadsheetUtilities
{
    ...
    /// symbols +, -, *, and /. Spaces are significant only insofar that they delimit tokens.  For example, 
    ///"xy" is a single variable, "x y" consists of two variables "x" and y; "x23" is a 
    ///single variable; and "x 23" consists of a variable "x" and a number "23".
    ...
    public class Formula
    {
                ...
                else if (double.TryParse(tokens[i], out double result) || tokens[i] == ")")
                {
                    if (tokens[i + 1] != "+" && tokens[i + 1] != "-" && tokens[i + 1] != "/" &&
                        ...
                        throw new FormulaFormatException("The given formula is invalid because all numbers," +
                         ...
                         tokens[i] != "(" && tokens[i] != ")" && double.TryParse(tokens[i], out double result2) &&
                         !isVar(tokens[i]))
                {
                    throw new FormulaFormatException("The given formula is invalid because all tokens must" +
                        " be either an operator, an opening or closing parenthesis, a number, or a variable");
                }
            }
        ...
        /// throws a FormulaFormatException with an explanatory Message. The associated normalizer 
        /// and validator are the second and third parameters, respectively. If the formula contains 
        /// a variable v such that normalize(v) is not a legal variable, throws a FormulaFormatException 
        /// with an explanatory message. If the formula contains a variable v such that isValid(normalize(v)) 
        /// is false, throws a FormulaFormatException with an explanatory message. Suppose that N is a method 
        /// that converts all the letters in a string to upper case, and that V is a method that returns true 
        /// only if a string consists of one letter followed by one digit. Then: new Formula("x2+y3", N, V) 
        /// should succeed new Formula("x+y3", N, V) should throw an exception, since V(N("x")) is false
        ...
        public Formula(String formula, Func<string, string> normalize, Func<string, bool> isValid)
        {
            ...
            for (int i = 0; i < tokens.Count; i++)
            {
                ...
                else if (isVar(tokens[i]) && !isVar(normalize(tokens[i])))
                {
        ...
        /// if it appears more than once in this Formula. For example, if N is a method that 
        /// converts all the letters in a string to upper case: new Formula("x+y*z", N, s => true).GetVariables() 
        /// should enumerate "X", "Y", and "Z" new Formula("x+X*z", N, s => true).GetVariables() should 
        /// enumerate "X" and "Z". new Formula("x+X*z").GetVariables() should enumerate "x", "X", and "z".
        /// </summary>
        ...
        /// variables in the string should be normalized. For example, if N is a method that 
        ...
        public override string ToString()
        {
            ...
            {
                if (isVar(tokens[i]))
                    returnString += normalizer(tokens[i]);
                else
        ...
        /// whether or not this Formula and obj are equal. Two Formulae are considered 
        /// equal if they consist of the same tokens in the same order. To determine 
        /// token equality, all tokens are compared as strings except for numeric tokens 
        /// and variable tokens. Numeric tokens are considered equal if they are equal after 
        /// being "normalized" by C#'s standard conversion from string to double, then back 
        /// to string. This eliminates any inconsistencies due to limited floating point precision.
        /// Variable tokens are considered equal if their normalized forms are equal, as 
        /// defined by the provided normalizer. For example, if N is a method that converts 
